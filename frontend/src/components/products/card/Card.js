  ...
  return (
    <div>
        {loading?(
            ...
            <Link to={`/products/${item?.id}`} className="w-full  min-h-[400px] border border-gray-200 rounded-lg shadow flex flex-col justify-between">
                <div className='mb-15 pb-15' >
                    <img className="p-8 rounded-t-lg w-[300px] h-[250px] object-contain" alt={item?.title} src={item?.thumbnail}  />
                </div>
                <div className="px-5  ">
                    <div>
                        <h5 className="text-2xl font-semibold tracking-tight text-gray-900 clamped-text">{item?.title}</h5>
                    </div>
